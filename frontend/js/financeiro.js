...
//CARREGAR CATEGORIAS SALVAS (guardadas no navegador, alem das padrao)
function carregarCategoriasSalvas() {
  try {
    const salvas = JSON.parse(localStorage.getItem(CHAVE_CATEGORIAS) || "[]");
    const jaExistentes = Array.from(selectCategoria.options).map(function (o) {
      return o.value;
    });
...
//GARANTIR OPCAO DE CATEGORIA (ex: vinda de uma despesa já existente, criada em outro navegador)
...
//GERAR OPCOES DE MES (todos os meses do ano corrente + qualquer mês, de qualquer ano, que já tenha serviço cadastrado)
...
async function carregarDados() {
  ...
  try {
    const dados = await chamarApi("/api/financeiro?ano=" + ano + "&mes=" + mes);
    const servicos = dados.servicos || [];
    const despesas = dados.despesas || [];

    atualizarKpis(servicos, despesas);
    renderizarTabelaServicos(servicos);
...
//ATUALIZAR KPIS
function atualizarKpis(servicos, despesas) {
  let receita = 0;
  let pendente = 0;
  let pagos = 0;
  let pendentes = 0;
...
function renderizarTabelaServicos(servicos) {
  ...
  servicos.forEach(function (s) {
    const nomeCliente = (s.cliente && s.cliente.nome) || "—";
...
function renderizarTabelaDespesas(despesas) {
  ...
  document.querySelectorAll(".btn-deletar-despesa").forEach(function (btn) {
    btn.addEventListener("click", async function () {
      ...
      try {
        await chamarApi("/api/financeiro/despesas/" + id, { method: "DELETE" });
        carregarDados();
      } catch (e) {
        mostrarErro(e);
      }
  ...
  document.querySelectorAll(".btn-editar-despesa").forEach(function (btn) {
    btn.addEventListener("click", function () {
      const id = btn.getAttribute("data-id");
      const despesa = despesas.find(function (d) {
...
btnCancelarDespesa.addEventListener("click", fecharPainel);
...
formDespesa.addEventListener("submit", async function (evento) {
  ...
  try {
    await chamarApi(caminho, { method: metodo, body: dadosDespesa });
    fecharPainel();
    carregarDados();
  } catch (e) {
    mostrarErro(e);
