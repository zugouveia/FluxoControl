...
const painelServicosCliente = document.getElementById(
  "painel-servicos-cliente",
);
...
const corpoTabelaServicosCliente = document.querySelector(
  "#tbl-servicos-cliente tbody",
);
...
//PREENCHER SELECT DE ANOS (ano atual + 4 anteriores)
const anoAtualParaSelect = new Date().getFullYear();
for (let i = 0; i < 5; i++) {
  const opt = document.createElement("option");
  opt.value = String(anoAtualParaSelect - i);
  opt.textContent = String(anoAtualParaSelect - i);
...
//CARREGAR DESPESAS DO ANO (via /api/relatorios, que ja soma isso no backend — evita recalcular despesa no front)
...
function renderizarModoAno(ano) {
  ...
  const margem =
    totalReceita > 0 ? Math.round((totalLucro / totalReceita) * 100) : 0;
  ...
  document.getElementById("sum-luc").className =
    "resumo-valor " + (totalLucro >= 0 ? "valor-positivo" : "valor-negativo");
...
function renderizarModoMes(ano, mes) {
  ...
  desenharTabelaMeses(
    [{ mes: mes, receita: receita, despesas: despesas, lucro: lucro }],
    ano,
  );
...
function renderizarModoCliente(ano, mes) {
  ...
  document.getElementById("titulo-cliente-servicos").textContent =
    clienteSelecionadoNome;

  const servicosDoCliente = servicosFiltrados(ano, mes).filter(function (s) {
  ...
  document.getElementById("cli-total-recebido").textContent =
    formatarMoeda(recebido);
  document.getElementById("cli-total-servicos").textContent = total;
  document.getElementById("cli-ticket-medio").textContent =
  ...
  const servicosOrdenados = servicosDoCliente.slice().sort(function (a, b) {
    return new Date(b.dataInicio) - new Date(a.dataInicio);
  });

  servicosOrdenados.forEach(function (s) {
    const data = new Date(s.dataInicio).toLocaleDateString("pt-BR", {
      timeZone: "UTC",
    });
    const evento = s.evento || "—";
...
//DESENHAR GRAFICO DE BARRAS (receita x despesa por mes)
function desenharGrafico(mesesDados) {
  ...
  mesesDados.forEach(function (mm) {
    ...
    col.innerHTML =
      '<span class="bar-tip">' +
      labelReceita +
      '</span><div class="bar-track-cima"><div class="bar-fill bar-cima ' +
      (mm.receita > 0 ? "bar-verde" : "") +
      '" style="height:' +
      hReceita +
      '%"></div></div><div class="bar-zero"></div><div class="bar-track-baixo"><div class="bar-fill bar-baixo ' +
      (mm.despesas > 0 ? "bar-vermelha" : "") +
      '" style="height:' +
      hDespesa +
      '%"></div></div><span class="bar-tip">' +
      labelDespesa +
      '</span><span class="bar-m">' +
      nomesMeses[mm.mes - 1] +
      "</span>";
...
//DESENHAR TABELA DE MESES
function desenharTabelaMeses(mesesDados, ano) {
  corpoTabelaMeses.innerHTML = "";

  const comDados = mesesDados.filter(function (mm) {
    return mm.receita > 0 || mm.despesas > 0;
  });

  if (comDados.length === 0) {
    corpoTabelaMeses.innerHTML =
      '<tr><td colspan="4" class="cell-empty">Nenhum dado para ' +
      ano +
      ".</td></tr>";
    return;
  }
...
function desenharTopClientes(servicos) {
  ...
  if (lista.length === 0) {
    listaClientesEl.innerHTML =
      '<p class="cell-empty block-pad">Sem dados para este período.</p>';
    return;
  }
  ...
  lista.forEach(function (c, idx) {
    ...
    div.innerHTML =
      '<div class="cli-left"><span class="cli-num">' +
      (idx + 1) +
      '</span><div><p class="cli-nome">' +
      c.nome +
      '</p><p class="cli-sub">' +
      c.servicos +
      " serviço" +
      (c.servicos !== 1 ? "s" : "") +
      '</p></div></div><span class="cli-total">' +
...
anoSel.addEventListener("change", carregarDados);
mesSel.addEventListener("change", renderizar);
